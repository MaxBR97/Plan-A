...
describe('Configuration Flow Tests', () => {
    ...
    // test('ConfigureSetsAndParamsPage - modify and persist aliases', async () => {
    //     const { rerender } = await performUpload();

    //     // Navigate to sets and params page
    //     rerender(
    //         <TestWrapper>
    //             <ConfigureSetsAndParamsPage />
    //         </TestWrapper>
    //     );

    //     // Wait for page to load
    //     await waitFor(() => {
    //         expect(screen.getByText(/configure sets and parameters/i)).toBeInTheDocument();
    //     });

    //     // Find and modify a set alias
    //     const setCard = screen.getByText('set1').closest('.item-card');
    //     const setAliasInput = within(setCard).getByLabelText(/alias/i);
    //     fireEvent.change(setAliasInput, { target: { value: 'NewSetAlias' } });
    ...
    //     await waitFor(() => {
    //         const updatedSetCard = screen.getByText('set1').closest('.item-card');
    //         const updatedAliasInput = within(updatedSetCard).getByLabelText(/alias/i);
    //         expect(updatedAliasInput).toHaveValue('NewSetAlias');
    //     });
    // });
}); 